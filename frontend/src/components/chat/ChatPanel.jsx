...
export default function ChatPanel({ group, currentUserId, onRefresh, onBack }) {
  ...
  const handleLeave = async () => {
    ...
    try {
      ...
      onRefresh(); onBack();
    } catch (err) { dispatch(addToast({ type: 'error', message: err.response?.data?.message || 'Failed to leave' })); }
  };
  ...
  const handleDelete = async () => {
    ...
    catch (err) { dispatch(addToast({ type: 'error', message: err.response?.data?.message || 'Failed to delete' })); }
  };
  ...
  const handleRegen = async () => {
    ...
    catch (err) { dispatch(addToast({ type: 'error', message: err.response?.data?.message || 'Failed to regenerate' })); }
  };
